...
class ApiServices {
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        if (response.statusCode == 200) {
          ...
          await storageKey.write(key: 'access_token', value: loginModel.token);

          Fluttertoast.showToast(msg: "Login successfully");
          Get.to(() => const HomeScreen());
        } else {
          debugPrint("Hello Kahase");
  ...
  Future registerAuth(String email, String password, String name, String phone,
  ...
  Future<DashBoardModel> dashBoardApi(BuildContext context) async {
    late DashBoardModel dashBoardModel;
    try {
      String? token = await storageKey.read(key: 'access_token');
      print(token);
      final result = await InternetAddress.lookup("google.com");
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        var url = Uri.parse(StringHelperForApiUrl.baseUrl + "dashboard");
  ...
  Future<GetProductDetails> productDetailsApi(
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        var url = Uri.parse(
            StringHelperForApiUrl.baseUrl + "product/$productId");
        final response = await http.get(url, headers: {
          "Accept": "Application/json",
          "Authorization": "Bearer $token",
        });
        ...
        print(decoded);
        if (response.statusCode == 200) {
          getProductDetails = GetProductDetails.fromJson(decoded);
          print(getProductDetails.toJson());
          Fluttertoast.showToast(msg: "Product Detail");
        } else {
          Fluttertoast.showToast(msg: "${decoded['msg']}");
        }
      } else {
